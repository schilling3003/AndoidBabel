...
- Phase: Round 5 in progress — tabletop mode toggled from the main conversation app bar and language list expanded to all Moonshine STT/TTS languages
- Commit under evaluation: (pending push) on `devin/round1-scaffold-ui`
- Overall evidence-backed score: not rated / 100 (real engines compile and launch
  on an x86_64 emulator; Round 5 emulator smoke test passed; release APK compiles for arm64 and is signed for device-side install, but no physical-device or model-asset measurements yet)
- Release gates: Build, lint, unit tests, and Round 5 emulator smoke test pass;
...
| 5 | (pending push) | Main-app tabletop toggle and expanded language list | not rated | not rated | `Language` enum gains `supportsStt`/`supportsTts` and adds Vietnamese, Ukrainian, German, French, Hindi, Italian, Dutch, Portuguese, Russian, Turkish; `ModelDownloadManager` generates STT specs only for STT languages and TTS specs for all TTS languages with per-language default voices; `LanguageDropdown` takes a `filter` predicate; `SettingsScreen` source dropdown filters to STT languages and target dropdown shows all TTS languages; `ConversationScreen` app bar adds a splitscreen icon to toggle tabletop mode and `BigSpeakButton` disables recording for non-STT languages; `ConversationViewModel.startRecordingIn` guards against non-STT languages; `./gradlew :app:compileDebugKotlin :app:lintDebug :app:testDebugUnitTest :app:assembleRelease` green; `app-release.apk` debug-signed and `apksigner verify` passes; emulator smoke test on `android-34` x86_64 passed: tabletop toggle in conversation app bar, settings source/target dropdowns, all 16 language voice-model rows, no crash | awaiting physical-device test |
